...
// +----------------------------------------------------------------------
// | 白衣素袖在2021-12进行修订，通过Mysql保存验证码而不是Session以更好的支持API访问
// +----------------------------------------------------------------------
...
class Captcha
{
    ...
    public function check($code, $id)
    {
        ...
        if ($this->authcode(strtoupper($code), $id) == $secode['code']) {
            $this->reset && Db::name('kefu_captcha')->where('key', $key)->delete();
            return true;
        }
    ...
    public function entry($id)
    {
        ...
        $key     = $this->authcode($this->seKey, $id);
        $captcha = Db::name('kefu_captcha')
            ->where('key', $key)
            ->find();
        // 绘验证码
        if ($captcha && $nowTime <= $captcha['expiretime']) {
            $captcha = $this->_writeText($captcha['captcha']);
        } else {
            $captcha = $this->_writeText();
            // 保存验证码
     ...
     */
    private function _writeText($captcha = false)
    {
        $code   = []; // 验证码
