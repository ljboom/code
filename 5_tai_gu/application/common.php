...
function getUpPrice($num1,$num2){
	...
	$price=$suiji/100;
	$price=floatval($str.$price);
	return $price;
}

function getDownPrice($num1,$num2){
	//70%概率产生正数，30%概率产生负数
	$suiji=rand(1,10);
	if($suiji<=$num1*10){
		//正数
...
function iskaipan(){
	...
	$now=time();
	$w=intval(date("w"));	//0-6
	if($opentime>$closetime){	//如果开盘时间是今晚到明天凌晨

		//判断当前有没有过24点
		$s=intval(date("H"));
		
		if($s>12 && $s<24 && $w>0 && $w<6){	//12点之前
			$open_datetime=strtotime(date("Y-m-d ".$opentime));
			...
			if($w>1 && $w<=6){
				$close_datetime=strtotime(date("Y-m-d ".$closetime));
...
if(!function_exists('send_http')){
     ...
     * @return bool|string
     */
    function send_http($url, $params, $method = 'GET', $header = array(), $multi = false){
        $opts = array(
            CURLOPT_TIMEOUT        => 10,
