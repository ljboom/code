...
(function($, window, document) {
    ...
    var createInput = function(placeholder , type) {
        return '<div class="' + CLASS_POPUP_INPUT + '"><input type="' + (type || 'text') + '" autofocus placeholder="' + (placeholder || '') + '"/></div>';
    };
    ...
    var createPrompt = function(message, placeholder, title, btnArray, callback, type,itype) {
        if (typeof message === 'undefined') {
            return;
        } else {
        ...
        if (!$.os.plus || type === 'div') {
            return createPopup(createInner(message, title || '提示', createInput(placeholder, itype)) + createButtons(btnArray || ['取消', '确认']), callback);
        }
