    ...
    <div class="record_outer">
    	<table>
    		<tr>
    			...
    			<th>时间</th>
					<th>用户名</th>
    			<th> 充值总额(<?php echo ($tixian); ?>)</th>
    		
    			<th>提现总额(<?php echo ($chongzhi); ?>)</th>
    		</tr>
            <?php if(is_array($results)): $i = 0; $__LIST__ = $results;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$i): $mod = ($i % 2 );++$i;?><tr>
             
                    <td><?php echo date('m-d H:i',strtotime($i['time']));?></td>
					   <td><?php echo getUsername($i['id']);?></td>
                    <td><?php echo ($i["sum"]); ?></td>
                 
                    <td> <?php echo ($i["tixiansum"]); ?></td>
