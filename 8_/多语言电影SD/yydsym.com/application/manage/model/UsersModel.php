...
class UsersModel extends Model
{
    ...
    public function lockAccountt()
    {
        ...
        $userInfo = $this->field('username')->where('id', $param['uid'])->find();
        //更新 云管家
        $val = ($param['value'] == 0) ? 0 : 1;
        $updateRes = $this->where('id', $param['uid'])->update([
            'is_housekeeper' => $val,
            'housekeeper_time' => strtotime(date("Y-m-d", time())) + 365 * 24 * 3600,
        ]);
        if (!$updateRes) return '修改失败';
        //添加操作日志
        $actionStr = $param['value'] == 0 ? '锁定' : '解锁';
        //model('Actionlog')->actionLog(session('manage_username'),'将账号'.$userInfo['username'].$actionStr,1);
    ...
    public function artificialBatch()
    {
        ...
        foreach ($data as $key => $value) {
            ...
            $newUserTotal = model('UserTotal')->where('uid', $userId)->inc('balance', $value[$priceKey]);
            ...
            model('Actionlog')->actionLog(session('manage_username'), '通过批量人工存提为' . $value[$usernameKey]  . $value[$priceKey] . '元');
        }
