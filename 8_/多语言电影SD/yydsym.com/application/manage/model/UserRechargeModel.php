...
class UserRechargeModel extends Model{
	...
	public function rechargeList(){
		...
		$resultData = $this->field('ly_user_recharge.*,users.username,t.dailixian')
		    ->join('users','ly_user_recharge.uid = users.id')
		    ->join('UserTeam t', 'ly_user_recharge.uid = t.uid')
		    ->where($where)
		    ->order('ly_user_recharge.add_time','desc')
		...
		//查询全部代理
		//$ids = model('Users')->field('id,username')->where('user_type', 1)->select();
		foreach ($resultData as $key => &$value) {
			// 状态颜色
			$value['stateColor'] = $stateColor[$value['state']];
			//分页统计
			$pageTotal['countMoney'] += $value['amount'];
			//$value['dailixian'] = $this->dailixian($value['uid']);
			/*
			$res = model('UserTeam')->where('team', $value['uid'])->select();
			if($res) foreach($res as $rk = $rv){
    ...
    public function dailixian($uid)
    {
        $dlx = model('UserTeam')->where('uid', $uid)->where('team',$uid)-value('dailixian');
            
        if($dlx){
            $username = model('Users')->where('id',$dlx)->value('username');
            return $username;
        }
    }
