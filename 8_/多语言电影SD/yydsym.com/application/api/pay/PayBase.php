...
abstract class PayBase
{
     ...
     * @param $data array|string 请求数据
     * @param $type string 请求类型支持 form-data  / json
     * @param $header array
     * @param $closeSSl boolean 是否关闭SSL检查
     * @return string|bool
     */
    protected function _post($url, $data, $type = 'form-data', $header = [], $closeSSl = true)
    {
        $ch = curl_init();
        ...
        if ($type == 'json') {
            curl_setopt($ch, CURLOPT_HTTPHEADER, array_merge($resHeader, ['Content-Type: application/json; charset=utf-8']));
            curl_setopt($ch, CURLOPT_POSTFIELDS, is_array($data) ? json_encode($data, JSON_UNESCAPED_UNICODE) : $data);
        } else {
            if ($resHeader) curl_setopt($ch, CURLOPT_HTTPHEADER, $resHeader);
