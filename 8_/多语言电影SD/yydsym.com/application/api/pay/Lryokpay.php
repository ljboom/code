...
class Lryokpay extends PayBase
{
    const PAY_URL = 'https://cnmss.lryok.com/ty/orderPay';
    const PAYOUT_URL = 'https://sruis.lryok.com/withdraw/singleOrder';
    ...
    public function getConfig($param)
    {
        if ($param == 'busi_code') {
            ...
            if ($type > 0) {
                return config('pay.lryokpay.t' . $type . '.' . $param);
            }
        }
        return config('pay.lryokpay.' . $param);
    }
    ...
    {
        $oUser = Db::name('ly_users')->where('id', $op_data['uid'])->find();
        ...
        if(isset($op_data['busi_code'])) $busi_code = $op_data['busi_code'];
        $data = [
        ...
        $log_file = APP_PATH . 'pay_lryokpay.log';
    ...
    public function create_payout(array $oinfo, array $blank_info): bool
    {
        $data = [
            ...
            'mobile_no' => str_replace(" ", "", $blank_info['tel']),
            'summary' => 'shop',
        ...
        if (config('default_country') == 'PER') {
            $data['province'] = $blank_info['document_id'];
        }
        if (config('default_country') == 'COL') {
            $data['remark'] = '123456789';
            $data['province'] = $blank_info['document_id'];
        }
