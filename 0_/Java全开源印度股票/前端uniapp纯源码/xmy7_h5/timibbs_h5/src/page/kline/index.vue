<template>
  <div class="kline_detail_page">
      ...
      <div class="detail_title">
        <div class="ti_cont">
          <div class="top_back">
            ...
            <div class="right_title">
              ...
              <div class="b_t">
                <span v-if="isQh==0">{{ singDetails.code }}</span>
                <span v-else>{{ singDetails.futuresCode }}</span>
              </div>
            </div>
          </div>
              ...
              <div class="top_price">
                ...
                <div class="right">
                  ...
                  <span v-if="$store.state.userInfo && kLineDetails.if_zhishu != '0'" style="white-space: nowarp;">
                    {{ kLineDetails.type == 'us' ? '$' + $store.state.userInfo.enableFuturesAmt  :
                        kLineDetails.type == 'hk' ? 'HK$' + $store.state.userInfo.hkEnableAmt :
                        kLineDetails.type == 'my' ? 'Rm' + $store.state.userInfo.myEnableAmt :
                        kLineDetails.type == 'in' ? 'Rs' + $store.state.userInfo.inEnableAmt :
                        kLineDetails.type == 'vn' ? '₫' + $store.state.userInfo.vnEnableAmt :
                          '฿' +
        ...
        <div class="real_time_detail">
          <div class="left_now_price">
            <div class="top_now">
              <span v-if="isQh==0" :class="singDetails.nowPrice - singDetails.preclose_px < 0 ? 'price green' : 'price red'">{{
                  singDetails.nowPrice
              }}</span>
              <span v-else :class="singDetails.nowPrice - singDetails.lastClose < 0 ? 'price green' : 'price red'">{{
                  singDetails.nowPrice
              }}</span>
            </div>
            <div class="bottom_now">
              <div>
                ...
                <span v-else-if="isQh==0">{{ (singDetails.nowPrice - singDetails.preclose_px).toFixed(2) }}</span>
                <span v-else>
                  {{ (singDetails.nowPrice - singDetails.lastClose).toFixed(2) }}
                </span>
              </div>
              <div class="tew" v-if="isQh==0"
                ...
                <span v-else>({{ singDetails.nowPrice - singDetails.preclose_px < 0 ? '+' : '' }}
                  ...
                  <span>({{ singDetails.hcrate ? singDetails.hcrate : '0' }}%)</span>
              </div>
              <div class="tew" v-else
                :class="singDetails.nowPrice - singDetails.lastClose < 0 ? 'number green' : 'number red'">
                <span v-if="singDetails.nowPrice == 0">-</span>
                <span v-else>({{ singDetails.nowPrice - singDetails.lastClose < 0 ? '+' : '' }}
                  {{ ((Number(singDetails.nowPrice) - Number(singDetails.lastClose)) / Number(singDetails.lastClose) * 100).toFixed(2)}}%)</span>
              </div>
            </div>
          </div>
          <div class="right_ets">
            <div class="tops">
              <div class="lefts topes">
                ...
                <span v-if="isQh==0" :class="singDetails.nowPrice - singDetails.preclose_px < 0 ? 'number green' : 'number red'">{{
                    singDetails.open_px
                }}</span>
                <span v-else :class="singDetails.nowPrice - singDetails.lastClose < 0 ? 'number green' : 'number red'">{{
                    singDetails.todayOpen
                }}</span>
              </div>
              <div class="rights topes">
                ...
                <span :class="singDetails.nowPrice - singDetails.lastClose < 0 ? 'number green' : 'number red'">{{
                    singDetails.maxPrice
                }}</span>
              </div>
            </div>
            <div class="bottoms">
          ...
          <div class="left_s">
            ...
            <div class="ets">
              <div class="left_sell1">
                ...
                <span> {{ (Math.random()*(
                Number(singDetails.nowPrice) + Number(Number(singDetails.nowPrice)*0.01) -
                  Number(singDetails.nowPrice) - Number(Number(singDetails.nowPrice)*0.01) + 1)
                  + Number(singDetails.nowPrice)- Number(Number(singDetails.nowPrice)*0.01)).toFixed(2)
               }}</span>
              </div>
              <div class="right_sell1">
                <span>{{  (Math.random()*98).toFixed(0) }}</span>
              </div>
            </div>
            <div class="ets">
              <div class="left_sell1">
    ...
    <div class="btns">
      <div class="lefts">
        <div class="left_ca" @click="option()" v-if="isQh==0">
          <img src="../../assets/img/meishoucang.svg" alt v-if="isOptionOpt === false" />
          <img src="../../assets/img/shoucangle.png" alt v-else />
        </div>
        <div class="right_xx" @click="handleJj()">
          <a href="#"><img src="../../assets/img/xiaoxi.svg" alt /></a>
        </div>
      </div>
      <div class="rights" v-if="isQh==0">
        <div class="buy_btn" >
      ...
      <div class="rights" v-else>
        <div class="buy_btn" >
          <div class="top_buy" @click="goBuyQh(0)">
            <span>{{ $t('hj84') }}</span>
          </div>
          <div class="bottom_buy">
            <span>{{ singDetails.nowPrice }}</span>
        ...
        <div class="sell_btn" @click="goBuyQh(1)">
          <div class="top_sell">
            <span>{{ $t('hj85') }}</span>
          </div>
          <div class="bottom_sell">
...
export default {
  ...
  methods: {
    ...
    async getQhDetail () {
      let opts = {
        futuresGid: this.kLineDetails.code
      }
      let data = await api.querySingleMarket(opts)
      if (data.status === 0) {
        this.singDetails = data.data;
      } else {
        Toast(data.msg)
      }
    },
    ...
    async getSingDetails() {
      ...
      //console.log(this.kLineDetails,777)
      let opts = {
        code: this.kLineDetails.gid,
      ...
      await api.getSingleStock(opts).then(res => {
        if (res.status === 0) {
          ...
          //console.log(this.singDetails,99)
          ...
          // if (this.kLineDetails.if_zhishu == '1') {
          //   this.getHknews();
          // } else if (this.kLineDetails.if_zhishu == '2') {
          //   this.getHknews();
          // } else if (this.kLineDetails.if_zhishu == '3') {
          //   this.getHknews();
          // } else if (this.kLineDetails.if_zhishu == '4') {
          //   this.getHknews();
          // } else if (this.kLineDetails.if_zhishu == '5') {
          //   this.getHknews();
          // }else {
          //   this.getNohknews();
          // }
    ...
    goBuyQh(index) {
      this.$router.push({
        ...
        query: {
          ...
          stockType: this.kLineDetails.type,
          type: 0,
          futuresGid: this.kLineDetails.gid,
          id: this.singDetails.id,
          name: this.kLineDetails.name,
          if_us: this.kLineDetails.if_us,
        }
