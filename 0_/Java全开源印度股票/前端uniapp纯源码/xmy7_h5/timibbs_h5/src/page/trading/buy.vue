        ...
        <div class="tabs_content">
          ...
          <div class="tr_rs">
            <div class="top_bzz" style="padding-bottom: 0.2rem;">
              ...
              {{ stockType == 'us' ? $t('hj102') + '($)' : stockType == 'hk' ? $t('hj102') + '(HK$)' : stockType == 'my' ? $t('hj102') + '(Rm)' : stockType == 'in' ? $t('hj102') + '(Rs)' :stockType == 'vn' ? $t('hj102') + '(₫)' : $t('hj102') + '฿'
              }}</span>
              <span style="text-align: right;">{{ stockType == 'us' ? $t('hj103') + '($)' : stockType == 'hk' ? $t('hj103') + '(HK$)' : stockType == 'my' ? $t('hj103') + '(Rm)' : stockType == 'in' ? $t('hj103') + '(Rs)' :stockType == 'vn' ? $t('hj103') + '(₫)' : $t('hj103') + '฿'
              }}</span>
            </div>
            <div class="bottom_bzz">
              ...
              <span v-if="$store.state.userInfo.userIndexAmt != undefined && stockType == 'us'" style="white-space: nowarp;">
                {{ '$' + $store.state.userInfo.enableAmt }}
              </span>
              <span v-if="$store.state.userInfo.userIndexAmt != undefined && stockType == 'hk'" style="white-space: nowarp;">
                {{ 'HK$' + $store.state.userInfo.hkEnableAmt }}
              </span>
              <span v-if="$store.state.userInfo.userIndexAmt != undefined && stockType == 'my'" style="white-space: nowarp;">
                {{ 'Rm' + $store.state.userInfo.myEnableAmt }}
              </span>
              <span v-if="$store.state.userInfo.userIndexAmt != undefined && stockType == 'th'" style="white-space: nowarp;">
                {{ '฿' + $store.state.userInfo.thEnableAmt }}
              </span>
              <span v-if="$store.state.userInfo.userIndexAmt != undefined && stockType == 'in'" style="white-space: nowarp;">
                {{ 'Rs' + $store.state.userInfo.inEnableAmt }}
              </span>
              <span v-if="$store.state.userInfo.userIndexAmt != undefined && stockType == 'vn'" style="white-space: nowarp;">
...
export default {
  ...
  methods: {
    ...
    async gdBuy() {
      ...
      this.buying = false
      if (data.status === 0) {
        this.$store.commit('elAlertShow', { 'elAlertShow': true, 'elAlertText': this.$t('hj326'), 'elAlertType': 'success' });
        this.getUserInfo()
        this.$router.push('/warehouse?index=1')
      }else if( data.msg.indexOf('請先登錄') > -1 ){
        this.$store.commit('elAlertShow', { 'elAlertShow': true, 'elAlertText': this.$t('hj386') });
      }else if( data.msg.indexOf('购买数量小于') > -1 ){
        this.$store.commit('elAlertShow', { 'elAlertShow': true, 'elAlertText': this.$t('hj404') });
      }else if( data.msg.indexOf('购买数量大于') > -1 ){
        this.$store.commit('elAlertShow', { 'elAlertShow': true, 'elAlertText': this.$t('hj405') });
      }else if( data.msg.indexOf('请勿重复挂单') > -1 ){
        this.$store.commit('elAlertShow', { 'elAlertShow': true, 'elAlertText': this.$t('hj406') });
      }else {
    ...
    async gpBuy(opts) {
      ...
      if (data.status === 0) {
        ...
        }else if( data.msg.indexOf('请先实名认证') > -1 ){
          this.$store.commit('elAlertShow', { 'elAlertShow': true, 'elAlertText': this.$t('hj381') });
        }else if( data.msg.indexOf('账户已被锁定') > -1 ){
          this.$store.commit('elAlertShow', { 'elAlertShow': true, 'elAlertText': this.$t('hj380') });
        }else if( data.msg.indexOf('系统设置错误') > -1 ){
          this.$store.commit('elAlertShow', { 'elAlertShow': true, 'elAlertText': this.$t('hj390') });
        }else if( data.msg.indexOf('股票代码错误') > -1 ){
          this.$store.commit('elAlertShow', { 'elAlertShow': true, 'elAlertText': this.$t('hj397') });
        }else if( data.msg.indexOf('下单失败，不能超过可用资金') > -1 ){
          this.$store.commit('elAlertShow', { 'elAlertShow': true, 'elAlertText': this.$t('hj357') });
        }else if( data.msg.indexOf('周末或节假日不能交易') > -1 ){
          this.$store.commit('elAlertShow', { 'elAlertShow': true, 'elAlertText': this.$t('hj395') });
        }else if( data.msg.indexOf('当前股票不能交易') > -1 ){
          this.$store.commit('elAlertShow', { 'elAlertShow': true, 'elAlertText': this.$t('hj398') });
        }else if( data.msg.indexOf('频繁交易') > -1 ){
          this.$store.commit('elAlertShow', { 'elAlertShow': true, 'elAlertText': this.$t('hj399') });
        }else if( data.msg.indexOf('购买数量小于') > -1 ){
          this.$store.commit('elAlertShow', { 'elAlertShow': true, 'elAlertText': this.$t('hj403') });
        }else if( data.msg.indexOf('购买数量大于') > -1 ){
          this.$store.commit('elAlertShow', { 'elAlertShow': true, 'elAlertText': this.$t('hj404') });
        }else if( data.msg.indexOf('报价0，请稍后再试') > -1 ){
          this.$store.commit('elAlertShow', { 'elAlertShow': true, 'elAlertText': this.$t('hj400') });
        }else if( data.msg.indexOf('ST和已退市的股票不能入仓') > -1 ){
          this.$store.commit('elAlertShow', { 'elAlertShow': true, 'elAlertText': this.$t('hj401') });
        }else if( data.msg.indexOf('当前股票已涨停不能买涨') > -1 ){
          this.$store.commit('elAlertShow', { 'elAlertShow': true, 'elAlertText': this.$t('hj402') });
        }else if( data.msg.indexOf('购买金额小于') > -1 ){
          this.$store.commit('elAlertShow', { 'elAlertShow': true, 'elAlertText': this.$t('hj403') });
        }else {
          this.$store.commit('elAlertShow', { 'elAlertShow': true, 'elAlertText': this.$t('hj327') });
        }
