      ...
      <div class="login_forms">
        <div class="top_forms">
         ...
         <!-- <div class="password">
          ...
          <div class="allmobile">
            <el-select class="areadiv" v-model="area">
              <el-option value="1"></el-option>
              <el-option value="852"></el-option>
              <el-option value="66"></el-option>
              <el-option value="91"></el-option>
            </el-select>
            <el-input :placeholder="$t('hj27')"  v-model="phone" clearable @input="handleInput()" maxlength="11">
            </el-input>
          </div>
         ...
         <!-- <div class="captcha">
            <el-input type="text" :placeholder="$t('hj25')" v-model="code" ></el-input>
            <div class="getcodebtn" v-if="codeshow" @click="checkCodeBox">{{$t('hj25')}}</div>
            <div v-if="!codeshow" class="getcode">{{count}}s</div>
          </div>-->
        </div>
    ...
    async getcode() {
      ...
      if (isNull(this.phone)) {
        ...
        // } else {
          ...
          let result = await api.getCode({ phoneNum: this.phone ,area: this.area})
          if (result.status === 0) {
            const TIME_COUNT = 60
            if (!this.timer) {
        ...
        if (this.userPassword !== this.rePassword) {
          ...
          if (data.status === 0) {
            ...
            if( data.msg.indexOf('验证码错误') > -1 ){
              this.texts = this.$t('hj424')
            }else if(data.msg.indexOf('代理不存在') > -1){
              this.texts = this.$t('hj426')
            }else if(data.msg.indexOf('手机号已注册') > -1){
              this.texts = this.$t('hj427')
            }else{
              this.texts = this.$t('hj425')
            }
