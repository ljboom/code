  ...
  <div class="wrapper">
    ...
    <div class="detail-part">
      <div class="index-name">
        <p>{{kLineDetails.name}} <span>{{kLineDetails.code}}</span></p>
      </div>
      <div class="clearfix">
    ...
    <div class="box-tab">
      <div class="tab-title">
        <span class="circle"></span>{{$t('hj338')}}
      </div>
      <div class="tab-con">
...
<!--          <li class="auto" v-show="!selectNumber">
            <input @keyup="changeAutoNumber" v-model="autoNumber" type="text">{{$t('hj339')}}
          </li> -->
        </ul>
        <p class="clearfix">
          <span class="pull-left">{{$t('hj340')}} {{detail.minNum}} {{$t('hj339')}}</span>
          <span class="protem pull-right">{{$t('hj341')}} {{detail.maxNum}} {{$t('hj339')}}</span>
        </p>
        <!-- <p class="clearfix">
    ...
    <div class="box-tab">
      <div class="tab-title">
        <span class="circle"></span>{{$t('hj342')}}
      </div>
      <div class="tab-con">
        <ul class="radio-group clearfix">
    ...
    <div class="box-tab">
      <div class="tab-con">
        <p class="text-left page-part">
          <span class="">${{selectNumber?selectNumber*detail.futuresStandard:autoNumber*detail.futuresStandard}}</span>
          <span class="pull-right">{{$t('hj343')}}:{{(settingInfo.buyMaxPercent * $store.state.userInfo.enableFuturesAmt).toFixed(2)}}</span>
        </p>
        <p class="clearfix">
          <!-- <span class="pull-right">最小购买金额{{settingInfo.buyMinAmt}}元</span> -->
          <span class="pull-left protem">{{$t('hj344')}}:{{total?total:0}} {{detail.coinCode}} </span>
        </p>
    ...
    <div class="footer-btn">
      <div class="total">
        <p class="pay">{{$t('hj293')}}<span class="protem">${{cnyTotal?cnyTotal:0}}</span></p>
        <p class="account">({{$t('hj345')}}:${{$store.state.userInfo.enableFuturesAmt}})</p>
      </div>
      <mt-button :disabled='buying' class="btn-red" size="small" type="danger" @click="toInquiry()">{{$t('hj346')}}</mt-button>
    </div>
...
export default {
  ...
  data () {
    return {
      ...
      selectCycle: '20',
      numberList: [
        { label: this.$t('hj348'), value: '20' },
        { label: this.$t('hj349'), value: '30' },
        { label: this.$t('hj350'), value: '50' },
        { label: this.$t('hj351'), value: '80' },
        { label: this.$t('hj352'), value: '100' },
      ],
      selectNumber: '',
      autoNumber: '',
      type: [
        { label: this.$t('hj353'), value: '0' },
        { label: this.$t('hj354'), value: '1' }
      ],
      selectType: '',
      // number:0,// 股
      // price:0,// 股价格
  ...
  methods: {
      ...
      if (data.status === 0) {
        ...
        if(this.$store.state.userInfo !== undefined && this.$store.state.userInfo !== null && this.$store.state.userInfo.phone !== '' && this.$store.state.userInfo.siteLever !== null){
            ...
            for (let i = 0; i < this.$store.state.userInfo.siteLever.split('/').length; i++) {
              ...
              let item = { label: val + this.$t('hj369'), value: val }
    ...
    async getDetailInfo () {
      let opts = {
        futuresCode: this.kLineDetails.code ? this.kLineDetails.code : ''
      }
    ...
    async getDetail () {
      let opts = {
        //futuresGid: this.kLineDetails.futuresGid ? this.kLineDetails.futuresGid : ''
        futuresGid: this.kLineDetails.code ? this.kLineDetails.code : ''
      }
    ...
    async toInquiry () {
      ...
      if (!this.agree) {
        ...
        if (data.status === 0) {
          ...
          this.$router.push('/orderlist?index=4')
        } else if (data.msg.indexOf('交易时段内') > -1) {
            Toast(this.$t('hj113'))
            ...
            Toast(this.$t('hj380'))
        } else if (data.msg.indexOf('产品不存在') > -1) {
            Toast(this.$t('hj407'))
        } else if (data.msg.indexOf('该产品不能交易') > -1) {
            Toast(this.$t('hj398'))
        } else if (data.msg.indexOf('最低不能低于') > -1) {
            Toast(this.$t('hj404'))
        } else if (data.msg.indexOf('最多不能高于') > -1) {
            Toast(this.$t('hj405'))
        } else if (data.msg.indexOf('系统设置错误') > -1) {
            Toast(this.$t('hj390'))
        } else if (data.msg.indexOf('频繁交易') > -1) {
            Toast(this.$t('hj399'))
        } else if (data.msg.indexOf('账户资金不足') > -1) {
            Toast(this.$t('hj389'))
        } else if (data.msg.indexOf('不能超过最大购买比例') > -1) {
            Toast(this.$t('hj408'))
        } else {
          Toast(this.$t('hj327'))
        }
