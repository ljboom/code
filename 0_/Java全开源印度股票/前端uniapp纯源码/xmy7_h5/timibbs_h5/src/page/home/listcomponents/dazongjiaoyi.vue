<template>
    <div>
        ...
        <div class="dbox">
            <div class="daz"><span class="d1">{{$t('hj39')}}</span><span class="d2">{{$t('hj81')}}</span><span class="d3"></span></div>
        </div>
        ...
        <van-popup v-model="show" round position="bottom">
            <div class="boxd">
                <div class="boxh"> {{$t('hj261')}} <span @click="show = false"></span></div>
                <h5>{{$t('hj130')}}</h5>
                <h6>{{ currentItem.price ? currentItem.price : '' }}</h6>
                <div class="erty tghj"><input :placeholder="$t('hj262')" type="number" class="inpy" v-model="num"
                        @input="num = num.replace(/^(0+)|[^\d]+/g, '')"><a>{{$t('hj117')}}</a></div>
                <p class="plm"><span>{{$t('hj263')}}</span><a>{{ currentItem.price ? (currentItem.price * num * 100).toFixed(2) :
                        '0.00'
                }}</a></p>
