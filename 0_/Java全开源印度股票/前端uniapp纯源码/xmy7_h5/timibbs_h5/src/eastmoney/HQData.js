...
HQData.RequestHistoryData=function(data, callback)
{
    ...
    let socketId = symbol.split("@")[1]
    let stockCode = symbol.split("@")[0]
    let type = ""
    let qhtype = ""
    let newtype = ""
    //console.log(stockCode,socketId)
    ...
    //console.log(999,period,qhtype);
    if(socketId === 'undefined'){
      $.ajax({
          url: `https://qhkline.dimetrades.com/getkine?symbol=${stockCode}&cmd=history&period=${qhtype}`,
          type: "GET",
          success: function (recvData) {
              let hqChartData = { code: 0, data: [] };
              hqChartData.symbol = symbol;
              hqChartData.name = symbol;
              recvData.list.forEach(v =>{
                  // let today = new Date(v['ctm']);
                  // let date =
                  //     today.getFullYear() * 10000 +
                  //     (today.getMonth() + 1) * 100 +
                  //     today.getDate();
                  //let date = v['ctm'] * 1
                  let today = new Date(dateFormat('YYYY-mm-dd HH:MM:SS',v['ctm']))
                  let date =
                      today.getFullYear() * 10000 +
                      (today.getMonth() + 1) * 100 +
                      today.getDate();
                  var time = today.getHours() * 100 + today.getMinutes();
                  let yClose = v['close'] * 1
                  let open = v['open'] * 1
                  let high = v['high'] * 1
                  let low = v['low'] * 1
                  let close = v['close'] * 1
                  let vol = 0
                  let amount = 0
                  let newItem = [date, yClose, open, high, low, close, vol, amount,time];
                  //console.log(newItem)
      ...
      $.ajax({
          //url: `https://kline.dimetrades.com/kline?pid=${socketId}&interval=${newtype}`,
          url: `https://api.investing.com/api/financialdata/${socketId}/historical/chart/?interval=${type}&pointscount=140`,
          type: "GET",
          success: function (recvData) {
              let hqChartData = { code: 0, data: [] };
              hqChartData.symbol = symbol;
              hqChartData.name = symbol;
              recvData.data.forEach(v =>{
                  let today = new Date(v[0]);
                  // let today = new Date(v["t"]);
                  let date =
                      today.getFullYear() * 10000 +
                      (today.getMonth() + 1) * 100 +
                      today.getDate();
                  ...
                  // let vol = v["vo"]
                  let amount = 0
                  let newItem = [date, yClose, open, high, low, close, vol, amount];
                  hqChartData.data.push(newItem);
              })
              callback(hqChartData);
          }
...
HQData.RequestHistoryMinuteData=function(data, callback)
{
    ...
    //console.log(888,period,qhtype)
    if(socketId === 'undefined'){
      $.ajax({
          url: `https://qhkline.dimetrades.com/getkine?symbol=${stockCode}&cmd=history&period=${qhtype}`,
          type: "GET",
          success: function (recvData) {
            //console.log(recvData,999)
              let hqChartData = { code: 0, data: [] };
              hqChartData.symbol = symbol;
              hqChartData.name = symbol;
              recvData.list.forEach(v =>{
