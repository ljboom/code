...
HQData.RequestMinuteData = function (data, callback) {
    ...
    //马来，泰国，印度无分时
    if (symbol.includes("my") || symbol.includes("th") || symbol.includes("in")) {
        return;
    }
    $.ajax(
        {
...
HQData.GetMinuteApiUrl = function (symbol, dayCount) {
    ...
    var url = `https://push2his.eastmoney.com/api/qt/stock/trends2/get?fields1=f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12,f13&fields2=f51,f52,f53,f54,f55,f56,f57,f58&secid=${internalSymbol.Market}.${internalSymbol.Symbol}&ndays=1&iscr=0&iscca=0`
...
HQData.GetKLineApiUrl = function (symbol, period, right, option) {
    console.error("symbol:" + symbol + ",period:" + period + ",right:" + right + ",opt:" + option);
    ...
    //马来，泰国，印度,系统新接口
    if (symbol.includes("my") || symbol.includes("th") || symbol.includes("in")) {
        var newsymbol = symbol.replaceAll("_.", ".");

        var newklit = internalPeriod;
        if (internalPeriod == 102) {
            newklit = "W";
        } else if (internalPeriod == 103) {
            newklit = "M";
        } else if (internalPeriod == 1) {
            newklit = "5";
        } else if (internalPeriod == 101) {
            newklit = "D";
        }
        url = `http://127.0.0.1:8095/api/stock/kline?beg=20190909&end=20500101&ut=fa5fd1943c7b386f172d6893dbfba10b&rtntype=6&secid=${newsymbol}&klt=${newklit}&fqt=${internalRight}`;
    }
 ...
 * @param right
 * @param option
 * @returns {{Symbol, InternalSymbol: {Symbol: *, Market: number}, Right, Period, Url: (string|*)}}
 * @constructor
 */
HQData.GetMinuteKLineApiUrl = function (symbol, period, right, option) {
    ...
    if (symbol.includes("my") || symbol.includes("th") || symbol.includes("in")) {
        ...
        console.log(newklit)
        url = `https://gp2api.lyi618.net/api/stock/kline?beg=20190909&end=20500101&ut=fa5fd1943c7b386f172d6893dbfba10b&rtntype=6&secid=${newsymbol}&klt=${newklit}&fqt=${internalRight}`;
    }
