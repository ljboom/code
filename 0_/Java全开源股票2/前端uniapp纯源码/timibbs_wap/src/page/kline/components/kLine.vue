          ...
          <div class="periodWrap kline_periodWrap" ref="kline_periodWrap">
            ...
            <div class="btnGroup" style="position:relative">
              <div
                ...
                :key="item.ID"
                @click="item.ID == 3 ? changeRightContent('minute') : OnClickKLinePeriodMenu(index, item, 'kline')"
              >
                <span>{{ item.Name }}</span>
              </div>
...
export default {
  ...
  created() {
    ...
    //判断当时是否为马来，泰国，印度，如果是就隐藏分时，1分钟
    if(this.Symbol.includes("my") || this.Symbol.includes("th") || this.Symbol.includes("in")){
      //默认下标
      var idsToRemove = [3,4]
      this.KLinePeriodIndex = 0;
      this.KLinePeriodMenu = this.KLinePeriodMenu.filter(item => !idsToRemove.includes(item.ID));
    }
