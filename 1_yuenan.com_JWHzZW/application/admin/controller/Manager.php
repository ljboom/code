...
class Manager extends Adminbase
{
    ...
    public function index()
    {
        if ($this->request->isAjax()) {
            ...
            foreach($_list as $k => $v){
                ...
                $_list[$k]['status'] = empty($v['status']) ? '禁用' :  '正常';
                $_list[$k]['bid_name'] = $this->modelClass->where('id',$v['bid'])->value('nickname');
            }
            $total  = count($_list);
            // dump($_list);
    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            //echo $this->auth->getRoleid();
            $this->assign('roleid', $this->auth->getRoleid());
            return $this->fetch();
        }
    }
    ...
    public function del()
    {
        ...
        //     }
        // }
        $this->modelClass->where('id',$id)->update(['del'=>2,'status'=>0]);
        $this->success("删除成功！");
        // $this->error('没有权限删除！');
    }
