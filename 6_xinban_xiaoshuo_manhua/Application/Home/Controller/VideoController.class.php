...
class VideoController extends HomeController {
    ...
    public function bookinfo(){
    	...
    	$info = M('video')->where("id={$yid}")->find();
    	if(empty($info)) {
    	...
    	M('video')->where("id={$yid}")->setInc('reader', 1);
	...
	public function getJino(){
		if(IS_POST){
			...
			$yid = I('post.yid');
			$start = ($page - 1)*20;
			$list = M('video_episodes')->where(array('yid'=>$yid))->order('ji_no asc')->limit($start,20)->select();
			$info = M('video')->find(intval($yid));
		    if($list){
			   $html = "";
			   foreach($list as $v){
				    $money = $v['money'];
             ...
             if(IS_POST){
               ...
               $read = M('book_ys_read')->where(array('user_id'=>$this->user['id'],'yid'=>$yid,'ji_no'=>$jino))->find();
                ...
                if(!$read && $this->user['vip']!=1){
                    $info = M('video_episodes')->where(array('yid'=>$yid,'ji_no'=>$jino))->find();
   ...
   public function paynumPlay(){
    ...
    public function user_add_book_shelf_ajax() {
    	...
    	if($user_id > 0) {
    		$binfo = M('video')->where("id={$yid}")->find();
    		$old = M('mh_collect')->where(array("mhid"=>$yid,"user_id"=>$this->user['id'],'type'=>"ys"))->find();
    		if(empty($old)) {
    			$ins = array(
    					'mhid'		=> $yid,
