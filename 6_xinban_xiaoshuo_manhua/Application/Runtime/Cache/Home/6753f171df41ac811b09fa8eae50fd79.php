...
<div class="rt-bar" style="position: fixed">
    <div class="row">
        <div class="col icon">
            <a href="<?php echo U('Video/videoinfo');?>&bid=<?php echo ($bid); ?>">
...
<div class="read-footer">
    ...
    <div class="row">
        ...
        <div class="col">
            <a href="<?php echo U('Video/videoinfo');?>&bid=<?php echo ($info["bid"]); ?>&isji=1" style="-webkit-tap-highlight-color: rgba(0,0,0,0);">
    ...
    <div class="control clearfix">
        <div class="item prev">
        <?php
 $before = $info['before']; if($before <= 1) { $before = 1; } $next = $info['next']; $money = M('video_episodes')->where(array('ji_no'=>$next,'bid'=>$binfo['id']))->getField('money'); $money = $money?$money:$_site['jimoney']; $read = M('video_read')->where(array('ji_no'=>$next,'bid'=>$binfo['id'],'user_id'=>$user['id']))->find(); ?>
...
<a href="<?php echo U('Video/jubao',array('rid'=>$binfo['id'],'type'=>'xs'));?>" style=" position: fixed;right: 5px;bottom: 20%;width: 50px;height: 50px;border-radius: 50%;background-color: rgba(0,0,0,.4);display: block;">
...
<script type="text/javascript">
    ...
    $(function(){
		...
		$('.ds li').click(function(){
			if(confirm("确定要打赏么？")){
				...
				$.post("<?php echo U('Video/mxSend');?>",{sid:sid,mxid:mxid,type:"xs"},function(d){
					if(d){
						if(d.status){
							dsHide();
							alert(d.info);
	...
	function before(bid,before){
		location.href="<?php echo U('Video/inforedit');?>&bid="+bid+"&ji_no="+before+"&isNight="+isNight+"&fsize="+fsize+"&ps="+ps;
	}
	...
	function next(bid,next,pay){
		if(subscribe !="1"){
			if(parseInt(sub) <= parseInt("<?php echo ($_GET['ji_no']); ?>")){
				...
				location.href="<?php echo U('Video/inforedit');?>&bid="+bid+"&ji_no="+next+"&isNight="+isNight+"&sub=<?php echo ($_GET['sub']); ?>&fsize="+fsize+"&ps="+ps;
			}
		}else{
			location.href="<?php echo U('Video/inforedit');?>&bid="+bid+"&ji_no="+next+"&isNight="+isNight+"&sub=<?php echo ($_GET['sub']); ?>&fsize="+fsize+"&ps="+ps;	
		}
...
function chapter_dianzan(bid, ji_no) {
	var url1 = '<?php echo U("Video/chapter_dianzan_ajax");?>';
...
function collect(bid){
	var url = '<?php echo U("Video/user_add_video_shelf_ajax");?>';
	var data = {bid:bid}
	var fun1 = function(data2){
        if(data2.status == 1) {
            if(data2.tag == 1) {
