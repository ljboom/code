...
?>
  <tr align="center" class="tdbottom" onMouseOver="javascript:this.className='tdbottomover';" onMouseOut="javascript:this.className='tdbottom';"> 
    ...
    <td height="25"><?php echo $val['h_point2Day'] , '×' , $val['h_num'] , '=' , ($val['h_point2Day'] * $val['h_num']) , '金币'; ?></td>
    ...
    <?php if(!$val['h_isEnd']){
			...
			if($mustSettleDay >0){
		?>
		<a style="cursor:pointer;" onClick="javascript:hintandturn('确认要结算该会员的农场物品产币数据吗？','?clause=settle&username=<?php echo urlencode($val['h_userName']); ?>',true);">结算</a>
		<?php
			}
	} ?>
	   <?php if($val['h_isEnd']){  ?>
