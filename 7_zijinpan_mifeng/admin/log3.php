...
function main()
{
	...
	if(strlen($stype) > 0){
		switch($stype){
			case '金币转账(入)':
				$where .= " and h_type = '金币转账' and h_price > 0";
				break;
			case '金币转账(出)':
				$where .= " and h_type = '金币转账' and h_price < 0";
				break;
	...
	global $page;
	$list_num = 15;
	$total_count = $db->counter("`h_log_point2`", "1 = 1 {$where}", 'id');$page = (int)$page;$rowset = new Pager($total_count,$list_num,$page);$from_record = $rowset->_offset();$page_list = $rowset->link('?page=');
...
<table width="98%" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#FFFFFF" class="tableborder">
  <tr>
    <td height="25" colspan="11" align="center" class="tdtitle">金币流水明细</td>
  </tr>
  <tr align="center"> 
    <td class="tdtitle-title">类型</td>
    <td height="23" class="tdtitle-title">会员帐号</td>
    <td class="tdtitle-title">金币</td>
...
<option value="金币拍卖交易完成" <?php if($stype == '金币拍卖交易完成'){echo "selected";}?>>金币拍卖交易完成</option>
<option value="取消金币购买" <?php if($stype == '取消金币购买'){echo "selected";}?>>取消金币购买</option>
<option value="金币拍卖" <?php if($stype == '金币拍卖'){echo "selected";}?>>金币拍卖</option>
<option value="购买物品" <?php if($stype == '购买物品'){echo "selected";}?>>购买物品</option>
<option value="物品产币" <?php if($stype == '物品产币'){echo "selected";}?>>物品产币</option>
<option value="物品产币分红" <?php if($stype == '物品产币分红'){echo "selected";}?>>物品产币分红</option>
...
<option value="金币转账" <?php if($stype == '金币转账'){echo "selected";}?>>金币转账</option>
<option value="金币转账(入)" <?php if($stype == '金币转账(入)'){echo "selected";}?>>金币转账(入)</option>
<option value="金币转账(出)" <?php if($stype == '金币转账(出)'){echo "selected";}?>>金币转账(出)</option>
