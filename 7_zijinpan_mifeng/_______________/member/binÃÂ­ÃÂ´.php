	...
	while($rs_list = $db->fetch_array($result)){
		...
		if($rs1){
			if((intval($rs1['sumNum']) + $goodsIN[$rs_list['id']]) > $rs_list['h_allMaxNum']){
				echo $rs_list['h_title'] , '同时个能存在' , $rs_list['h_allMaxNum'] , '个，您当前最多个能购买' , ($rs_list['h_allMaxNum'] - intval($rs1['sumNum'])) , '个';
				exit;
			}
		}
		//判断今天购买是否超量
		...
		if($rs1){
			if((intval($rs1['sumNum']) + $goodsIN[$rs_list['id']]) > $rs_list['h_dayBuyMaxNum']){
				echo $rs_list['h_title'] , '每天最多个能购买' , $rs_list['h_dayBuyMaxNum'] , '个，您今天最多还能购买' , ($rs_list['h_dayBuyMaxNum'] - intval($rs1['sumNum'])) , '个';
				exit;
			}
		}
		//等级是否符合
